...
export const DeckTable: React.FC<DeckTableProps> = ({ decks, onDeckClick }) => {
    return (
        <div className="border border-zinc-200 rounded overflow-hidden">
            {/* Table Header */}
            <div className="bg-zinc-50 border-b border-zinc-200 px-2 py-1 grid grid-cols-[1fr_48px] sm:grid-cols-[1fr_48px_64px_48px_48px] gap-2 items-center">
                <div className="text-xs font-semibold text-zinc-600">Deck</div>
                <div className="hidden sm:block text-xs font-semibold text-zinc-600 text-right">New</div>
                <div className="hidden sm:block text-xs font-semibold text-zinc-600 text-right">Learning</div>
                <div className="hidden sm:block text-xs font-semibold text-zinc-600 text-right">Review</div>
                ...
                {decks.map((deck) => (
                    ...
                    >
                        ...
                        <div className="min-w-0">
                            <div className={cn(
                                "text-sm sm:text-xs text-zinc-900 truncate",
                                deck.isAllBooks && "font-semibold"
                            )}>
                                {deck.title}
                            </div>
                            {deck.author && (
                                <div className="text-xs sm:text-[10px] text-zinc-400 truncate">
                                    {deck.author}
                                </div>
                            )}
                        </div>

...
export const DeleteBookModal: React.FC<DeleteBookModalProps> = ({ bookId, onConfirm, onCancel }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/20 flex items-center justify-center z-50" onClick={onCancel}>
      <div
        className="bg-white rounded-md shadow-lg border border-zinc-200 p-5 max-w-md mx-4"
      ...
      >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-4 flex items-start gap-2">
              <input
                ...
                className="mt-0.5 w-3.5 h-3.5 text-red-600 rounded border-zinc-300 focus:ring-red-500"
              />
              <label htmlFor="confirm-delete" className="text-xs text-zinc-700 cursor-pointer">
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button
            onClick={onCancel}
            className="px-4 py-2 text-xs font-medium text-zinc-700 hover:bg-zinc-100 rounded transition-colors"
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            disabled={!confirmed}
            className="px-4 py-2 text-xs font-medium text-white bg-red-500 hover:bg-red-600 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Delete Book
          </button>
        </div>

...
<task type="auto">
  <name>Task 2: Add Dashboard translations</name>
  <files>public/locales/pt-BR/dashboard.json</files>
  <action>
Create new translation file for Dashboard page:

```json
{
  "_meta": "Dashboard page translations",
  "quickStudy": {
  ...
  "empty": {
    ...
    "message": "Importe seus highlights do Kindle para comecar a estudar.",
    "importButton": "Importar Highlights"
  }
}
```
  </action>
  <verify>JSON is valid: `cat public/locales/pt-BR/dashboard.json | python -m json.tool`</verify>
  <done>dashboard.json exists with all required translation keys</done>
</task>

<task type="auto">
  <name>Task 3: Register Dashboard namespace in i18n</name>
  <files>components/I18nProvider.tsx</files>
  <action>
Check if the i18n configuration automatically loads namespaces from the locales folder. If not, add 'dashboard' to the ns array in the i18n init configuration.

The i18next configuration typically auto-loads based on the files in the locales folder, so this may already work without changes. Verify by checking the i18n setup in I18nProvider.tsx.

If manual registration is needed, add 'dashboard' to the namespaces list.
  </action>
  <verify>Dashboard page can load translations: verify t('quickStudy.title') works in Dashboard.tsx</verify>
  <done>Dashboard namespace is available for translation</done>
</task>
...
3. Translations file is valid JSON

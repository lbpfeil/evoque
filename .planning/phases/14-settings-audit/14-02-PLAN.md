...
<task type="auto">
  ...
  <files>setupTests.ts, package.json</files>
  <action>
    1. Create `setupTests.ts` in the project root:
       ```typescript
  ...
  <verify>
    - `setupTests.ts` exists in project root
...
2. `setupTests.ts` exists in project root with jest-dom import and cleanup

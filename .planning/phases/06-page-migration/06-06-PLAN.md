...
must_haves:
  truths:
    ...
    - "All text-[10px] in TagSelector replaced with text-overline"
    - "Modal header typography uses token classes (text-heading for DialogTitle)"
    - "No arbitrary text-[Npx] values remain in any modal, popover, sidebar, nav, or tag component"
  artifacts:
    ...
    - path: "components/TagSelector.tsx"
      provides: "Token-aligned tag selector"
  key_links: []
---

<objective>
Migrate all modals, popovers, sidebar, bottom nav, and tag selector to token typography.
...
Output: All modals, Sidebar.tsx, BottomNav.tsx, TagManagerSidebar.tsx, and TagSelector.tsx fully aligned with token typography.
</objective>
...
<task type="auto">
  <name>Task 2: Migrate Sidebar, BottomNav, TagManagerSidebar, and TagSelector to token typography</name>
  <files>components/Sidebar.tsx, components/BottomNav.tsx, components/TagManagerSidebar.tsx, components/TagSelector.tsx</files>
  <action>
    **Sidebar.tsx:**
    ...
    **TagSelector.tsx:**
    1. Line 186: `text-[10px] font-medium` (tag badge text) -> `text-overline font-medium`
    That is the only arbitrary text value in TagSelector.
    ...
    Do NOT change TagSelector popover structure, search, or selection logic.
  </action>
  <verify>
    ...
    Grep components/TagSelector.tsx for `text-\[\d+px\]` -- should find zero occurrences.
    Run `npx tsc --noEmit 2>&1 | grep -iE "(Sidebar|BottomNav|TagManager|TagSelector)"` -- no TypeScript errors.
  </verify>
  <done>Sidebar, BottomNav, TagManagerSidebar, and TagSelector have zero arbitrary text-[Npx] values. All replaced with text-overline.</done>
</task>
...
- Zero `text-[Npx]` remain in any modal, sidebar, nav, or tag component
...
- TagSelector: 1 replacement
- All files compile with no TypeScript errors
- Zero arbitrary text-[Npx] remain across all 7 files
</success_criteria>
